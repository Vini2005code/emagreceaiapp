...
const FUNCTION_NAME = "analyze-food";
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: plan } = await adminClient.from("subscription_plans").select("scanner_daily_limit").eq("id", planId).maybeSingle();
    const dailyLimit = plan?.scanner_daily_limit || 3;
