...
import { Camera, Upload, Loader2, Flame, Beef, Wheat, Droplet, RotateCcw, Utensils, Scale, AlertCircle, CheckCircle2 } from "lucide-react";
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {result?.confianca_da_analise && (
                      <div className={`flex items-center gap-1 text-xs ${getConfidenceColor(result.confianca_da_analise)}`}>
                        ...
                        <span>Confiança: {result.confianca_da_analise}</span>
                      </div>
                    )}
                  </div>
                </div>
                    ...
                    <div className="pt-3 border-t space-y-3">
                      ...
                      {result.gorduras_ocultas_adicionadas && (
                        <div className="flex items-center gap-2 text-xs text-amber-600 bg-amber-50 dark:bg-amber-950/30 rounded px-2 py-1">
                          ...
                          <span>Gorduras ocultas (óleos/temperos): +{result.gorduras_ocultas_adicionadas}</span>
                        </div>
                      )}
                      ...
                      {result.referencias_de_escala && result.referencias_de_escala.length > 0 && (
                        <div className="text-xs text-muted-foreground">
                          <span className="font-medium">Referências de escala:</span> {result.referencias_de_escala.join(", ")}
                        </div>
                      )}
                    </div>
                  )}
